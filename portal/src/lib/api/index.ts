...
/**
 * Subscribes to the chat stream (GET .../chats/{chatId}/stream). No run_id; stream is keyed by chat.
 * Uses fetch + ReadableStream so Authorization header is sent. Parses SSE and calls onDelta for
 * each data event, onDone when the server sends the "done" event or the stream ends.
 * Call the returned cleanup to abort.
 */
...
/**
 * Subscribes to the run stream (GET .../runs/{runId}/stream). Prefer subscribeChatStream for Portal.
 */
