...
/**
 * Fetch a page of a space's audit trail, newest first.
 *
...
export async function getAuditEvents(
  ...
  const url =
    `${getApiBase()}/api/spaces/${encodeURIComponent(spaceId)}/audit-events` +
    (query ? `?${query}` : "")
  return requestJson<ApiAuditEventsResponse>(url, { headers: authHeaders(token) })
}

/**
 * Download a space's whole audit trail as a file.
 *
...
export async function exportAuditEvents(
  ...
  const url =
    `${getApiBase()}/api/spaces/${encodeURIComponent(spaceId)}/audit-events/export` +
    `?format=${format}`
  await downloadAuthenticated(url, token, `audit-${spaceId}.${format}`)
}
