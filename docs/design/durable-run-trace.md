...
// mcp_boundary — synthesized at construction beside sandbox_boundary, so a run
// states how its MCP transports were treated. On a worker, stdio is disabled by
// the unattended-worker profile. Transport kinds only, never a command or url;
// an absent record reads as "written before this existed", not "stdio allowed".
{"ts":"…","type":"mcp_boundary","mcp_stdio_disabled":true,
 "mcp_remote_transports":["http","sse"]}
...
agentapp.RunPrompt(ctx, sess, prompt, stream, approval, eventSink)
  ...
  │        then sandbox_boundary, mcp_boundary, context_sources, plugins
  │
