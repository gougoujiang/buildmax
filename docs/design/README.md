...
| [Workspace root and worktrees](workspace-root-and-worktrees.md) | step 5 product bet | Planned, nothing implemented: the root becomes session state and an agent creates, enters, and cleans up its own worktrees. Decisions are settled, D1-D10; §9 holds the prompt-cache measurement and whether listing alone keeps worktrees from accumulating |
