...
| [Team Secrets and run delivery](team-secrets.md) | R0 credential debt, R3 team surface | Designed, not started: no secret table, route, or runtime path exists. Settles delivery as run-level — an Agent revision declares environment variables, rendered credential files, or both — and records that a run can read what it was granted, so the boundary is Team ownership, per-Agent declaration, credential lifetime, and audit rather than confidentiality against the model. Answers Phase D3 of [plugin-team-distribution.md](plugin-team-distribution.md) |
