...
| [Agent-scoped sandbox policy](agent-sandbox-policy.md) | P0.5 follow-on | Backend implemented: agent-revision-scoped network/filesystem sandbox tiers, claim-time resolution and pinning, and `SandboxSurfaceWorker` selection. Portal selectors, the space default tier, and cluster-level verification are open |
