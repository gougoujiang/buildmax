...
| [Team membership lifecycle](team-membership-lifecycle.md) | P4 (roadmap R3) | Implemented end to end: team-scoped invitation bounded to existing accounts (account creation stays `system_admin`-only), role change, unilateral ownership transfer, member-scoped access recovery, and the Portal surfaces for all four |
| [Team Secrets and run delivery](team-secrets.md) | R0 credential debt, R3 team surface | Designed, not started: no secret table, route, or runtime path exists. A Secret is a Team-owned group of named items in one encrypted row, unversioned; consumption is configured on the Agent revision and delivered run-level as environment variables or rendered credential files. Records that a run can read what it was granted, so the boundary is Team ownership, per-Agent consumption, credential lifetime, and audit rather than confidentiality against the model. Answers Phase D3 of [plugin-team-distribution.md](plugin-team-distribution.md) |
