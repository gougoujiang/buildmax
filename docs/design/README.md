...
| [Workspace root and worktrees](workspace-root-and-worktrees.md) | step 5 product bet | Phases 1-2 shipped: the root is session state every tool reads per call, and an agent creates, enters, leaves, and removes its own worktrees under the containment, permission, and occupancy rules. Derived-configuration reload, hook events, and delegate worktrees are phases 3-5. Decisions are settled, D1-D10 |
