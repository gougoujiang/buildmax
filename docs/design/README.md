...
| [Issue agent access](issue-agent-access.md) | unscheduled | Decided, not implemented: an Agent reads and reports on the one Issue it is working, through two runtime tools scoped by construction. Status, assignment, and hierarchy stay out of the model's hands. Steps 1-5 are worker-plane work; step 6 is the local bridge |
