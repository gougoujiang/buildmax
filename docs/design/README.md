...
| [Graceful shutdown](graceful-shutdown.md) | P3 follow-on | Phase 1 shipped: the shutdown ladder, the draining state, watcher-stream drain and its Portal half, turn quiescing, managed terminal callbacks, HTTP timeouts, and the manifests. Phase 2 (a worker that honours SIGTERM) and phase 3 (a bounded scheduler stop) are open, so an interrupted run still says nothing until the stale-run reaper closes it |
