...
| [Evaluation and qualification](evaluation-system.md) | P0.6 | Local and worker slices shipped: the contract, both black-box adapters, deterministic/command/trace graders, preflight, repetition, paired comparison, and three tasks. The legacy harness is retired. Terminal-Bench 2.1 through Harbor is the accepted first external target; its adapter, conversation and deployment adapters, model-grader calibration, the holdout, and the Inspect spike are open |
