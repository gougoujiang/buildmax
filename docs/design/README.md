...
| [Workspace root and worktrees](workspace-root-and-worktrees.md) | step 5 product bet | Phase 1 shipped: the root is session state and every tool, the sandbox bind, the TUI footer, and `/diff` read it per call. The worktree lifecycle, derived-configuration reload, hook events, and delegate worktrees are phases 2-5. Decisions are settled, D1-D10 |
