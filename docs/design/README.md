...
| [Sandbox boundaries](sandbox-boundaries.md) | Local phases A–E implemented, process limits included; phase F's worker surface selection, production-pod verification, and downgrade marking shipped, its example-config docs and the §13 gaps — hook transports that do not consult `SandboxView`, no `sandbox overrides` command — open | [guide/sandbox.md](../guide/sandbox.md) |
