...
| [Issue agent access](issue-agent-access.md) | unscheduled | Shipped on the worker plane: `GetIssue` and `ReportToIssue`, scoped by construction, behind two run-token routes. Status, assignment, and hierarchy stay out of the model's hands. Artifact references in `GetIssue` are deferred on an ownership question. The local surfaces are blocked on §11's authorship question, not merely unbuilt |
...
| [Hook system](hook-system.md) | 16 events and 4 transports implemented | [guide/hooks.md](../guide/hooks.md) |
