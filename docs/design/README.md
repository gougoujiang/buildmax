...
| [Workspace root and worktrees](workspace-root-and-worktrees.md) | step 5 product bet | Phases 1-3 shipped: the root is session state every tool reads per call, an agent creates, enters, leaves, and removes its own worktrees under the containment, permission, and occupancy rules, and hooks, skills, subagents, MCP, and the `AGENTS.md` layer follow a move. Hook events, Desktop display, and delegate worktrees are phases 4-5. Decisions are settled, D1-D10 |
