...
Observed 2026-09-13 on `main` cceba61c: `./make e2e desktop-ui` exited 1 with
`gui not built (missing gui/dist/index.js); run \`cd gui && npm ci && npm run
build\`` (`tools/mk/desktop_ui.go:107`). After `cd gui && npm ci && npm run
build`, `./make e2e desktop-ui core` passed. The refusal itself is intentional
