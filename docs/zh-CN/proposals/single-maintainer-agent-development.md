...
**已采纳的切片：** 主线规划现在使用仓库内的
[backlog](../../backlog/README.md)，面向外部贡献者的工作仍保留在 GitHub Issue，
并遵守“一项工作只在一处跟踪”的规则。backlog 文档是该规划与执行闭环的当前权威来源。
本提案只继续讨论下文中的自动化、租约、独立验收与基于度量扩展权限。后文历史文字中的
“可直接实现的 Issue”，应理解为按受众选择的就绪工作项：维护者与 Agent 的工作使用
backlog 任务，外部贡献使用 GitHub Issue，绝不同时存在于两处。
