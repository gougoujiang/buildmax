...
> **翻译说明：** 本文是[英文原文](../../design/tool-permissions.md)的简体中文派生翻译。**同步依据：** 英文原文 SHA-256 `9dc0bc4e7471c14809aec71e94b13f42fab4f5d40282f96a71149fdf53440112`。**同步状态：** 与该版本一致。若中英文存在语义冲突，以英文原文为准。
...
- 在 `help/` 增加任务导向页面，说明哪些操作会提示以及如何阻止。
...
- **现有沙箱策略层也有同样的缺陷。** §2 说明 Worker 无法读取 `<BUILDMAX_HOME>/policy.yaml`；`LoadPolicySandbox` 中记录的运维锁定只在本地 CLI 生效，所有者和用户文档见 `help/sandbox.md`。这是部署缺陷，不属于本设计，应单独解决：要么把文件纳入 `runGlobal`，要么按 §7 将沙箱策略移到服务器下发通道。本调查发现了它，但修复应作为独立工作跟踪。
