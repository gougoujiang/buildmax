...
2. **状态、Owner、Executor 和层级关系永远不可通过工具写入。** Agent 只陈述发生了什么；只有人可以判定工作处于什么状态。见 §6。
...
- 标题、描述、状态、Executor 类型（永远不含 Owner——那是对某个人的责任归属，这个接口没有理由看到它）；
...
`status`、`owner_id`、`executor_kind`、`executor_id` 和 `parent_issue_id` 不可被任何工具写入。创建一个子 Issue 也不是一个工具。
