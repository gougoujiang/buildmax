...
`internal/infra/db` 中的 `相关标识符ShareRow` 结构是事实的模式来源，
...
这里有危险的表面，并且 Unified-artifacts.md §6.3 强制下载 HTML
因为这个原因。该记录解除了预览表面的限制
在特定的安全模型下，而不是在一般情况下。
...
以及 API 的无 cookie 但仍然真实的表面。不透明来源沙箱
...
5. **共享缓存中的链接隐私。** 共享 HTML 内容可通过以下方式缓存：
   ...
   内容处理程序。目前发货编号为 `private, no-store`，与
