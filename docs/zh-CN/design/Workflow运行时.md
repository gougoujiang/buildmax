...
> **受众：** 贡献者、产品评审者与运维人员 · **状态：** 计划中——方向已经确定；当前实现仍然只是一个线性的、由回调驱动的雏形
...
本记录接受的是这套架构本身，而不是要求立即扩大范围。可靠性这个根基，必须先于新的 Workflow 控制流之前建成。静态图执行是一项 R5 能力，要等到 R0–R4 的运维与资格验证证据齐备之后才会被选中，除非某个具体部署能提供足够的证据和优先级,才会提前。动态扩展、人工等待、计划任务和入站事件,仍然留待更晚的阶段。
