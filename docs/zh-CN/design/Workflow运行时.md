...
> **受众：** 贡献者、产品评审者与运维人员 · **状态：** 计划中——方向已经确定；当前实现仍然只是一个线性的、由回调驱动的雏形。带保护的 compare-and-set Run/步骤转换与失败步骤原子收口已经交付；持久协调与重启恢复仍待完成
...
但这还不是本文档要设计的那个运行时。当前的定义只是一个带有静态提示词的、有序的 `steps` 数组。一次运行没有输入或结果契约，一个步骤无法绑定前一个步骤的输出，步骤上只保留一段 500 字符的摘要，下一个步骤是靠进程内的、尽力而为的终态回调来调度的。运行创建、步骤创建、Task 准入和步骤关联是各自独立的写操作。并发或重复的推进可能会造成重复的工作，服务器崩溃也可能让一次运行在其 TaskRun 已经进入终态之后仍然搁浅在原地。
...
本记录接受的是这套架构本身，而不是要求立即扩大范围。可靠性这个根基，必须先于新的 Workflow 控制流之前建成。静态图执行是一项 R5 能力，要等到 R0–R3 的 Beta 证据与后续产品证据证明其必要性后才会被选中；只有具体部署提出需求和优先级时才提前。动态扩展、人工等待、计划任务和入站事件仍然留待更晚的阶段。
