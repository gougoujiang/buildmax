...
Runtime Hook 系统已经实施，详细设计见[Hook 系统](./hook-system.md)。
...
每次运行都会持续写入运行时事件流，详细设计见[持久化运行轨迹](./durable-run-trace.md)。第一阶段在单一 `agentapp.RunPrompt` 入口写入有界、脱敏的轨迹，因此 CLI/TUI、Desktop、评估工具和 Worker 运行都能获得一致的轨迹，而不必由各界面重复实现。每次运行写入 `<DataDir>/sessions/<session_id>/traces/<run_id>.jsonl`，事件包括 `run_start`、`sandbox_boundary`、`llm_*`、`tool_*`、`context_compacted` 和 `run_end`；可通过 `BUILDMAX_TRACE_DISABLED` 关闭。
...
记忆必须有明确范围并可见。会话笔记和 todo 属于当前会话的工作记忆；共享 CLI/Desktop Project 身份和有限的 Project 记忆见[本地项目记忆](local-project-memory.md)。全局用户记忆、未来的 Space 记忆和可复用 Agent 记忆仍是独立范围，不能把 `AGENTS.md` 或 Agent 指令误当成记忆。
...
该问题承接已退役的 *Agent 执行策略* 提案。Worker 执行的四个方面已经确定，并在上文或[沙箱边界](./sandbox-boundaries.md)中描述：运行持有什么、运行在哪里、资源限制以及记录什么。尚未解决的是**谁拥有决定权**。边界不能只由部署脚本隐式决定：集群运维人员可能加固每个 Worker，也可能完全不加固，而系统没有定义请求的限制无法使用时应发生什么。过去沙箱并未覆盖所有界面，因此这个问题一直没有被明确提出。

其中一部分已经解决：Worker 如何访问*服务器*。Worker 控制 API 通过独立的 TLS listener 提供，由内部 Service 和 NetworkPolicy 保护，只允许带标签的 Worker Pod 访问，见[Worker API 网络边界](./worker-api-network-boundary.md)。这限制 Worker 到服务器的流量，但不决定 Worker 能否访问 Git 主机、软件包注册表或模型端点。
...
当前建议比“每层 Space 一个配置文件”更窄：重新启用 `SandboxConfig` 的 `Network`/`Filesystem` 轴，并保持其他轴和运维人员的 `policy.yaml` 上限不变。集群出站访问仍属于本节的开放问题，而不是本记录已解决的内容，见[Agent 沙箱策略](./agent-sandbox-policy.md)。
