...
> **翻译说明：** 本文是[英文原文](../../design/space-governance.md)的简体中文派生翻译。**同步依据：** 英文原文 SHA-256 `fe56bc28b5ca8a19f79ae53efdd649f5c6cda00d128427d96ff677c89e2420ff`。**同步状态：** 与该版本一致。若中英文存在语义冲突，以英文原文为准。
...
审计写入失败时记录错误并继续原操作，而不是让原操作失败。这是有意的取舍：让审计中断原操作，会把“记录失败”变成“业务失败”。这是实实在在的限制；`help/support.md` 应说明，轨迹记录的是数据库可用时发生的事情，而不是保证每个动作都留下记录。
...
事件在变更成功后写入。写入失败只记录错误，不让已成功的业务动作失败。该取舍在 §5.4 和 `help/support.md` 中明确说明。严格的 JSON 元数据方案已取消：没有元数据列，只有简短的 `Detail` 字符串。
