...
- 任务运行详情视图：Run Details 现在会展示轨迹记录的边界与解析后的 Plugin 固定版本。请求/解析后的层级对和 stdio MCP 处理仍未作为独立字段显示；阅读者已经不必直接翻阅轨迹文件才能看到实际边界。
