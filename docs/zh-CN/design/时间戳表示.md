...
> **翻译说明：** 本文是[英文原文](../../design/timestamp-representation.md)的简体中文派生翻译。**同步依据：** 英文原文 SHA-256 `b942ed9f6460d996ae63dec5ee2a1bdd739965b26529166bf041d492ffde7c04`。**同步状态：** 与该版本一致。若中英文存在语义冲突，以英文原文为准。
...
相关:[数据模型](../../contribute/architecture/data-model.md),[商店](../../contribute/architecture/store.md),[会议](../../contribute/conventions.md),[实体身份](实体身份.md) 为DSN数据库单一连贯的阿尔法方案变更的先例和[配置](../../reference/configuration.md)。
...
**存储库已经运行第二个会议.** 会议文件包含`CreatedAt time.Time`和RFC 3339字符串 (`internal/core/session/session.go`)； 痕迹和工作日志写RFC 3339纳米`ts` (`internal/infra/trace/record.go`).所以文件层已经选择了该记录为数据库提供的表示，并且它们之间的每个边界都转换.转换也不均:`trace/joblog.go`邮票在当地时间中标记相关标识符，而`trace/record.go`s[阅读中文镜像](时间戳表示.md)。 `time.Now()` `time.Now().UTC()`

现在的BuildMax是Alpha中的，并没有与发布的API或现有数据库相兼容，这就是使得现在的价格便宜，后来又昂贵的原因。 [实体身份](实体身份.md)

## 2.目标和非目标

**目标.** 每层一个表示，可以从该领域的含义而不是其邻居中导出.SQL即一个人可以在没有转换函数的情况下读写.次次精度.缺失表达为`NULL`，一次.每次写在 UTC，连接被固定，使数据库一致.一个连贯的方案改变没有双读路径。

**非目标.** 时间，配额，重试数量和代币数量是实时，不会改变 (§3,D7)。 会议和跟踪文件格式已经使用RFC 3339并且未被触及.订单语义学不会改变：微秒精度不是仅仅在时间标签上排序的许可证 (D6)。 没有数据库外键  [实体身份](实体身份.md) §8 拥有该决定，并且该记录不会重新打开。 没有时间区意识的"本地墙时间"类型： BuildMax 存储实时，用户的时间是表现问题。
