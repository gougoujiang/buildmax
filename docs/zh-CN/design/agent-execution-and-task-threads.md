...
> **受众：** 贡献者、产品设计者和操作员 · **状态：** 正在进行 — 所有权切换 (§13.1)，直接Agent 准入（第 13.2 节）、Task 线程后端和 Portal Task 页面（第 13.3 节）以及合成 Conversation 删除（第 13.4 节）已发货，每个都带有 MySQL 争用或浏览器证据。 §14 逐项准确跟踪已验证的内容和未完成的内容；不要在此处重复该列表。

相关：[产品愿景](product-vision.md),
[表面定位](surface-positioning.md),
...
- [7. Conversation 是独立的前景表面](#7-conversation-是独立的前景表面)
...
| Issue |共享工作和结果背景 |私有Agent历史或工人生命周期|
...
授权、配额。发送创建一个新的 TaskRun 并保留之前的回合
不可变的。
...
Agent 详细表面列出了该 Agent 的任务，最新的在前，带有状态，
