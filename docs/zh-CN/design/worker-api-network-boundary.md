...
本设计不增加默认拒绝 Worker 出站流量的策略。保留 Git、注册表、模型和对象存储访问，需要等待[信任工具链](trust-harness.md) §3.9 中尚未解决的域名感知出站决策。该决策确定并验证后，再在此处增加收窄的 Worker 出站规则。
...
路由 × 状态矩阵测试列出每条 Worker 路由允许的 TaskRun 状态，包括令牌在任务完成后继续使用的情况。这是叠加在网络边界之上的应用授权，符合[Space Secret 与运行交付](space-secrets.md) §7。
...
因此 Worker listener 要在公共 listener 之后关闭，而不是同时关闭。增加排空窗口必须符合 Pod 的 `terminationGracePeriodSeconds` 合约，见[优雅关闭](graceful-shutdown.md)。
