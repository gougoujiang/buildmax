...
> **翻译说明：** 本文是[英文原文](../../design/enterprise-deployment.md)的简体中文派生翻译。**同步依据：** 英文原文 SHA-256 `9a9dfb92abc0d26cdb347c1c23fe3c21f439fc73262e5b1bf2f496824420f5ad`。**同步状态：** 与该版本一致。若中英文存在语义冲突，以英文原文为准。
...
8. 是否针对至少一项架构更改执行了升级和回滚？
   `manual/support.md` 中的 N-1 承诺是代码遵循的规则，而不是
   运行任何人都执行过。
...
11. Kubernetes、MySQL和S3兼容存储的哪些版本构成
    支持矩阵？ `manual/support.md` 对界面和平台进行分级，但
