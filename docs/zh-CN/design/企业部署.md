...
> **翻译说明：** 本文是[英文原文](../../design/enterprise-deployment.md)的简体中文派生翻译。**同步依据：** 英文原文 SHA-256 `96e554277a890c0ec75189114251634c324482f4aea49ce1cbb4b991219c635f`。**同步状态：** 与该版本一致。若中英文存在语义冲突，以英文原文为准。
...
8. 是否针对至少一项架构更改执行了升级和回滚？
   `help/support.md` 中的 N-1 承诺是代码遵循的规则，而不是
   运行任何人都执行过。
...
11. Kubernetes、MySQL和S3兼容存储的哪些版本构成
    支持矩阵？ `help/support.md` 对界面和平台进行分级，但
