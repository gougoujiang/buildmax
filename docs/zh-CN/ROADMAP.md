...
R0–R2 优先，因为执行安全与状态正确性是所有 Server 功能的基础。
R3–R4 补齐操作与资格验证证据。这些是优先级，不代表每项都已有负责人正在开发。

### R0. 约束无人值守的 worker 执行

**部分实现。** 官方 worker 镜像已选择 worker 沙箱基线；Bash 隔离、进程限制、
hook 传输策略和后端自检已实现，并由部署冒烟测试覆盖。
MCP stdio 子进程与集群级网络出口仍在该边界之外。
worker API 已有独立监听器、TLS 支持和已交付的 Server 入站 NetworkPolicy；
整个 worker 的出站网络是另一项缺口。

**下一步：** 定义并实施 MCP 子进程边界；在候选环境验证已交付的 worker API 网络边界；
决定更广泛的 worker 出站网络策略。让运维界面能够清晰展示实际生效的沙箱策略。

**完成标准：** 受支持的 worker 配置强制实施文档声明的进程与网络边界，
必要的强制机制不可用时拒绝执行，并提供对应的部署证据。
可选 gVisor 配置必须用实际 worker 与沙箱探针验证后，才能宣称受支持或推荐使用。

设计：[信任保障](design/信任保障.md)、
[worker API 网络边界](../design/worker-api-network-boundary.md)、
...
| 执行边界与拓扑 | 验证受支持的沙箱、资源限制、hook/MCP 处理和 Server 拓扑。仅记录 `none` 边界的无限制 Bash 不合格。明确记录剩余 worker 出站网络与存储凭证限制。 |
