...
**还剩一个工程缺口。** 官方 worker 镜像已经选择并探测 worker 沙箱基线；
Bash 隔离、进程限制、hook 传输策略和 worker API 隔离已经实现，并由部署冒烟测试覆盖。
MCP stdio server 仍作为直接子进程启动，处在命令边界之外。

**下一步：** 让受支持的无人值守 worker 配置拒绝 stdio MCP，除非其子进程能够
在声明的沙箱边界内启动。用候选制品验证现有的 Bash、进程限制、hook 和 worker API
控制，并在诊断 TaskRun 的运维界面中展示实际解析出的沙箱与 MCP 处理方式。

**完成标准：** 受支持的 worker 配置所声明的边界之外没有 stdio MCP 子进程；
必要的强制机制不可用时失败关闭；实际边界可见；并且候选部署证据覆盖这些声明。
Pod 级目的地控制与外层 runtime 沙箱不属于 R0 或首个私有 Beta 门槛。
