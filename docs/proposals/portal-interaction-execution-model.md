...
model. TaskRun now records the `source_message_id` that caused it — bound per
turn, so the model cannot choose or omit what its work is attributed to — and
`GET /api/teams/{team_id}/task-runs/{task_run_id}` quotes that message next to
the instruction the worker was given. Two of the three problems are answered by
that:

1. Tier 1 may still omit a constraint, scope boundary, or acceptance condition
   while rewriting the request — but the omission is now visible.
2. Diagnostics can compare the input sent to the worker with the user's own
   wording.
3. Still open: the attachments, context references, and workspace state used to
   construct the request are not recorded, only the message. A message is the
   stable reference point those would hang from, not a substitute for them.
...
- ~~record the source message on TaskRun~~ — shipped, `task_run.source_message_id`;
- define and validate ExecutionSpec;
- ~~trace the relationship between original input and normalized instructions~~ —
  shipped as a route rather than a trace event: the run provenance route quotes
  the message next to the instruction;
- ~~record the source message for every continuation run~~ — shipped, each run
  records its own;
- ~~distinguish explicit run and automatic dispatch in provenance~~ — already
  true before this phase: `created_by_type` and `trigger_source` separate them,
  and the provenance route states which.

What remains is the Tier 1 half — budgets, the context it may read, and
ExecutionSpec. The intent-preservation half is done as far as a message goes;
attachments and context references are not recorded, and would hang off the
message rather than replace it.
