...
| [Client sessions and API credentials](client-sessions-and-api-credentials.md) | Trust and Security | Which credentials should interactive, native, and unattended clients receive? | Session-chain listing and revocation shipped; explicit session state, expiry, claims, secure storage, and self-service remain open |
| [Enterprise identity and access](enterprise-identity-and-access.md) | Trust and Security | How should a private deployment connect corporate identity to BuildMax Spaces and roles? | Nothing |
...
| [Local Issue work bridge](local-issue-work-bridge.md) | Local Experience | How should connected local surfaces work with Space Issues? | Most of phase 1 shipped; the durable Issue-to-Session link and later phases remain open |
...
| [Scheduled Agent execution](scheduled-agent-execution.md) | Product and Execution Model | Should a Space run an Agent on a recurring time schedule, and what is the smallest shape that delivers it on the Task plane? | Nothing; `ChannelCron` is an unbacked placeholder and no time trigger exists |
