...
| [Live delivery and Server replica topology](live-delivery-and-replica-topology.md) | The production manifest runs two Server replicas while every live coordination mechanism is process-local. What is the supported topology, and is Tier 2 live task output a commitment, given that its stream hub currently has no consumer? | Nothing. The durable path it contrasts with — `task_run.Output` and `task_result_delivery` — already exists |
