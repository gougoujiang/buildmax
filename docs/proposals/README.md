...
| [Execution modes, authority, and the two-tier model](execution-modes-and-authority.md) | Is "Tier" an agent-architecture boundary or an execution mode, and is BuildMax's defensible position deployability or verifiable authority? | Nothing of the direction. The substrate it argues is the real asset — `source_message_id`, `agent_revision`, audit, traces — already exists; the Tier 1 evidence gap it names is a live defect |
...
| [Tier 2 visibility in the Portal conversation](tier-2-visibility.md) | Is Tier 2 a user-facing concept in a conversation or an execution detail of Tier 1, and does the live Tier 2 output path get finished, distributed, or deleted? | Nothing of the direction. What it proposes to delete is built and unconsumed: the stream hub, the task SSE endpoint, and the worker's delta sender |
