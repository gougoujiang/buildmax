...
The next roadmap should not spend its first phases reintroducing new top-level nouns. Instead, it should make the existing system feel like the product described in [design/001-about-portal.md](../design/001-about-portal.md):
...
- Tier 1 conversation orchestration in `internal/service/conversation`
...
The product baseline should be understood as one shared Agent core with multiple
deployment and usage surfaces:

- local single-user execution through CLI and Desktop
- enterprise/team operation through Server and Portal
- background execution through worker task runs

The Portal does not replace the local Agent experience. It packages the same
Agent capability into a private, team-oriented platform.

Key anchors in code:

- shared runtime: `internal/agentapp`
- task-run runtime: `internal/agentapp/taskrun`
- Tier 1 orchestration: `internal/service/conversation`
- issue/workflow/task/quota application services: `internal/service/issue`, `internal/service/workflow`, `internal/service/task`, `internal/service/quota`
- worker execution pipeline: `internal/server/scheduler`, `internal/infra/workerclient`, and `internal/agentapp/taskrun`
- server scheduler and worker launch path: `internal/server/scheduler`
- worker API client/updater: `internal/infra/workerclient`
- team authz: `internal/server/handlers/team_authz.go`
- workflow lifecycle enforcement: `internal/service/workflow/service.go`
- issue flow UI: `portal/src/pages/IssueDetail.tsx`
- workflow detail UI: `portal/src/pages/WorkflowDetail.tsx`
- team-scoped files UI/API: `portal/src/features/files`, `internal/server/handlers/files.go`
...
- shared Agent runtime plus collaborative execution system

to:

- **privately deployable enterprise Agent platform with versioned AI-native work surfaces**

The product center should shift from:

- "we have local Agent execution and a team execution console"

to:

- "the same Agent core can be used locally or deployed for a company; teams express intent, agents change workspace state, and the system explains, tracks, governs, and can restore those changes"
...
| 5 | Shared-Team Governance 2.0 | `in_progress` | `design/021-phase-5-team-quota-foundation.md` | Team-scoped quota foundation is landed; audit/event visibility and human checkpoints remain later governance work. |
