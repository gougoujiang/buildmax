...
**Implementation status:** Phases 1–3 are implemented. Worker streams stdout to server (POST stream); server hub + GET SSE endpoint; Portal subscribes after createTaskRun and shows streaming reply until done, then refetches conversation.

---

## 0. User vs internal model (chat vs chat-run)

**Principle:** Chat run is an internal concept (server ↔ worker). The user is not exposed to “runs”; they only see “chat”: they start a chat on the New Chat page, then continue the conversation on the task detail page.
...
| **Streaming** | Portal gets stream **by task**: e.g. `GET .../tasks/{task_id}/stream` (no run_id in URL) | Worker still reports per run: `POST /api/worker/task-runs/{run_id}/stream`; server maps run → task and keys hub by task_id |
| **Swagger / APIs** | User-facing APIs use **chat** (and workspace); no task_run_id in Portal-facing paths | Worker APIs keep **task_run** in the path; these are internal and not exposed to the user |
...
2. **No session / conversation not found**  
   If the conversation API returns “not found” (e.g. no session file in blob yet), treat this as “chat just started.” The task detail page can show the user’s initial input first (already supported in the Portal) and optionally poll or subscribe to the stream until the first run produces a conversation.

3. **Stream by chat (user-facing)**  
   The Portal should get the stream **by chat**, not by run:
   - **User API:** e.g. `GET /api/workspaces/{workspace_id}/tasks/{task_id}/stream` (no run_id). The client subscribes to “this chat’s” stream; at most one run is active per chat, so this is well-defined.
   - **Existing run-scoped URL** (e.g. `GET .../chats/{task_id}/runs/{run_id}/stream`) can remain for backward compatibility or be deprecated in favor of the chat-scoped endpoint.

4. **Stream hub keyed by task_id**  
   The worker (each chat-run) still reports to the server using run-scoped endpoints, but the **stream hub** should be keyed by **task_id**, not task_run_id:
   - Worker: `POST /api/worker/task-runs/{run_id}/stream` with `{ "delta": "..." }` (unchanged).
   - Server: resolve run_id → task_id (e.g. via TaskRunStore); then `hub.Append(task_id, delta)` and `hub.Done(task_id)` when the run completes. So all run-level traffic is translated to chat-level keys in the hub.
   - User: `GET .../chats/{task_id}/stream` subscribes to `hub.Subscribe(task_id)`, receiving deltas for whichever run is currently active for that chat.
...
**Current implementation (Phases 1–3)** uses run-scoped hub keys and run-scoped user stream URL (`GET .../chats/{task_id}/runs/{run_id}/stream`). The Portal currently receives `task_run_id` from `createTaskRun` and subscribes using that run_id. **Next steps** are to: (1) allocate session_id at chat creation and pass it to the worker; (2) add chat-scoped stream endpoint and key the hub by task_id; (3) have the Portal subscribe by task_id only (no run_id in the URL).
...
| 3 | **Worker API (get run)** | Include `session_id` in the run/chat payload returned to the worker (e.g. `GET /api/worker/task-runs/{task_run_id}`). | **Done** | `GetTaskRunResponse.Chat.SessionID` is already returned (`worker_handlers.go`, `workerapi/types.go`). Value is nil until 1 is done. |
...
| 5 | **Stream hub** | Change hub to be keyed by **task_id** instead of task_run_id. | **Done** | `stream_hub.go`: interface and impl use `chatID` (Append/Buffer/Done/Subscribe all keyed by task_id). |
| 6 | **Server (worker stream)** | In `POST /api/worker/task-runs/{run_id}/stream`: resolve run_id → task_id (via TaskRunStore), then `hub.Append(task_id, delta)`. | **Done** | `postWorkerStreamHandler` calls `GetTaskRunWithChat` then `hub.Append(run.ChatID, req.Delta)`. |
| 7 | **Server (worker PATCH)** | When worker PATCHes run SUCCEEDED/FAILED: call `hub.Done(task_id)` (resolve run_id → task_id). | **Done** | `patchWorkerTaskRunHandler` resolves run → task_id and calls `hub.Done(run.ChatID)`. |
| 8 | **Server (user stream)** | Add `GET /api/workspaces/{workspace_id}/tasks/{task_id}/stream` (no run_id). Auth: workspace + chat ownership. Subscribe to `hub.Subscribe(task_id)`; send buffer then live deltas then "done". | **Done** | `getChatStreamHandler` in `stream_handlers.go`; route in `server.go`. |
| 9 | **OpenAPI / Swagger** | Document the new chat-scoped stream endpoint for user API. Keep worker stream endpoints as internal (or clearly marked). | **Done** | `openapi.json`: path `GET .../chats/{task_id}/stream` added. |
| 10 | **Portal** | After create chat or create run: subscribe to `GET .../chats/{task_id}/stream` (no run_id). No need to pass or store task_run_id for streaming. | **Done** | `subscribeChatStream(workspaceId, chatId, token, callbacks)` in `api/index.ts`; ChatDetail subscribes on mount and does not use run_id for streaming; follow-up only calls `createTaskRun`. |
...
**Summary:** All items implemented. **1, 2**: session_id at chat creation. **3, 4**: worker API returns and uses session_id. **5–7**: hub keyed by task_id; worker POST/PATCH map run → task_id. **8–9**: chat-scoped GET stream + OpenAPI. **10**: Portal subscribes by chat on mount; no run_id for streaming. **11**: initial input on 404 (unchanged).

**Ordering:** 1–2 (session at creation) first. Then 5–7 (hub keyed by task_id, worker POST/PATCH map to task_id), then 8–9 (user stream endpoint + docs) and 10 (Portal).
...
- **Server**: No endpoint that pushes deltas. `GET .../chats/{task_id}/conversation` returns the full session JSON in one response (session is read from blob/local after the run’s `global/` is uploaded).
- **Portal**: After `createTaskRun`, it polls `getChats` every 2s until status is SUCCEEDED/FAILED, then `refetchSession()` → `getChatConversation` to get the whole conversation. No incremental display.

### 1.3 Data flow today

```
User (Portal) → createTaskRun → Worker runs buildmax -p (CombinedOutput)
...
- **Worker**: Replace `CombinedOutput()` with streaming stdout (e.g. `cmd.StdoutPipe()` + goroutine). For each chunk/delta, POST to server, e.g. `POST /api/worker/task-runs/{task_run_id}/stream` with body `{"delta": "..."}`.
- **Server**:  
  - New endpoint: `GET /api/workspaces/{id}/tasks/{id}/runs/{run_id}/stream` returning `text/event-stream`. For each delta received from the worker for that run, write an SSE event to subscribers; on worker PATCH SUCCEEDED/FAILED, send a “done” event and close.  
  - In-memory (or Redis) store: run_id → subscribers and optional buffer.
- **Portal**: After `createTaskRun`, open `EventSource(streamUrl)` (auth via query param if needed). Append each event to the “current assistant” message; on “done”, refetch session once and stop the stream.
...
- **Portal**: After `createTaskRun`, poll conversation (or run status) more frequently (e.g. every 500 ms). Render the last message as “assistant typing…” with partial content; when status becomes SUCCEEDED, one final refetch.
...
- **DB**: Holds only **pointers and denormalized summary** for conversation:
  ...
  - `task_run`: `output` (full assistant text for that run), `session_id`, `status`, etc.
...
| **Server (worker API)** | New `POST /api/worker/task-runs/{task_run_id}/stream` (worker auth); accept `{"delta":"..."}`; append to run buffer and broadcast to SSE subscribers. | Small–medium |
...
| **Portal** | After createTaskRun, open SSE (or fetch+ReadableStream) to stream URL; render "current assistant" message that grows with each event; on "done", refetch conversation and close stream; reduce or replace polling. | Medium |
...
- **Deliverables**:  
  - Executor: `runBuildmaxCmd` streams stdout; new interface or callback to send deltas (e.g. `StreamSender` or extend updater); worker calls `POST /api/worker/task-runs/{task_run_id}/stream` for each chunk (or batched/flushed).  
  - Server: `POST /api/worker/task-runs/{task_run_id}/stream` (worker auth); hub that maps run_id → buffer, appends delta.  
...
- **Deliverables**:  
  ...
  - `GET /api/workspaces/{id}/tasks/{id}/runs/{run_id}/stream` (or equivalent) with user auth (workspace/chat/run); return `text/event-stream`; auth via query param or fetch with Authorization.  
...
- **Goal**: Chat detail view opens the stream after createTaskRun, shows assistant message growing in real time, then finalizes on "done".  
- **Deliverables**:  
...
- **Worker** POSTs deltas to `POST /api/worker/task-runs/{task_run_id}/stream`. The load balancer may send that request to **instance A**.
...
- **Section 0 refactor:** `internal/storage/entity/chat.go` (CreateChat, session_id); `internal/server/stream_hub.go` (key by task_id); `internal/server/worker_handlers.go` (POST stream, PATCH run → resolve to task_id); `internal/server/stream_handlers.go` (user GET stream by task_id); `internal/executor/worker_api.go`, `internal/workercmd/run.go` (worker receives and uses session_id).
