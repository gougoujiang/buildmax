...
Merges are merge commits: the branch's commits land on `main` under a merge
commit whose subject comes from the pull request title. A maintainer merging a
pull request is responsible for that subject reading well in `git log` — edit it
at merge time rather than letting a vague title land. A branch whose own history
is not worth keeping should be tidied before merge, not merged as-is. Keep the
reviewer-facing half of the description out of the commit body.
