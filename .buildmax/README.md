...
[manual/skills-and-subagents.md](../manual/skills-and-subagents.md) and
[manual/mcp.md](../manual/mcp.md), the other half being your personal
`~/.buildmax/`.
...
| Path | What it is |
|---|---|
| `mcp.json` | An MCP server definition pointing at `tools/mcp`, so `/mcp` in the TUI has something real to connect to. Discovery order and the `${WORKSPACE_ROOT}` expansion are in [manual/mcp.md](../manual/mcp.md). |
