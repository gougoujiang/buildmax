...
| `skills/smoke/` | `/smoke [level]` — the manual tool smoke test `./make smoke` drives. Levels 0–2 go from read-only tools to file I/O to network. |
| `skills/vibe/` | `/vibe` — a development-lifecycle workflow (start, clarify, design, code, done, commit, push). Optional; it is one maintainer's working style, not a project requirement. |
