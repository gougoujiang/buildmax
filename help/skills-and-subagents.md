...
Two ways to give the agent more capability without making every prompt longer: skills load instructions on demand, and subagents run separate agent runs.
...
`read_file`. See [Tools](tools.md) for the list.
...
- [Tools](tools.md) — exact tool names for the `tools:` field
- [Project instructions](project-instructions.md) — instructions that apply to every run instead
- [Hooks](hooks.md) — `subagent_start` / `subagent_stop` events
