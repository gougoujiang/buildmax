...
| 路径 | 行为与……完全一致 | 文档见 |
|---|---|---|
| `skills/<name>/SKILL.md` | `<workspace>/.buildmax/skills/` | [技能与子 Agent](技能与子代理.md) |
| `agents/<name>.md` | `<workspace>/.buildmax/agents/` | [技能与子 Agent](技能与子代理.md) |
| `mcp.json` | `<workspace>/.buildmax/mcp.json` | [MCP 服务器](MCP服务器.md) |
| `hooks.yaml` | `<workspace>/.buildmax/hooks.yaml` | [钩子](钩子.md) |
...
这约束的是插件从何而来，而不是它们可以做什么。一个确实加载了的插件可以做什么，属于[工具权限](工具权限.md)和[沙箱](沙箱.md)。
...
一个插件无法给自己授予权限。工具权限、钩子门控、沙箱和敏感路径检查对插件贡献的作用，与它们对你自己配置的作用完全一致——参见[工具权限](工具权限.md)和[沙箱](沙箱.md)。
...
每次运行都会记录它加载了哪些插件，对一个检出还会记录其提交以及工作树是否有未提交更改——参见[会话与追踪记录](会话与追踪.md)。

## 相关

- [CLI](命令行.md) —— 每一条 `buildmax plugin` 命令
- [技能与子 Agent](技能与子代理.md) —— 编写内容
- [MCP 服务器](MCP服务器.md) · [钩子](钩子.md) —— 另外两种
