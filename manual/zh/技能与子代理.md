...
工具名必须精确匹配——`Glob, Grep, Read, WebFetch`，而不是 `glob` 或 `read_file`。见 [工具](工具.md) 获取列表。
...
- [工具](工具.md) — `tools:` 字段的精确工具名
- [项目指令](项目指令与记忆.md) — 转而适用于每次运行的指令
- [钩子](钩子.md) — `subagent_start` / `subagent_stop` 事件
