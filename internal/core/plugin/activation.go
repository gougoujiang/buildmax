...
// ErrAlreadyActivated means the space already has an activation for this
// plugin. Moving to another release is a pin change, not a second activation,
// which is why the two are different calls.
var ErrAlreadyActivated = errors.New("this space has already activated this plugin")

// Curation is a space's answer to who fills its activation list.
//
...
// docs/design/plugin-space-distribution.md §4.1.
...
const (
	...
	// that crosses spaces is operator eligibility, not a space's housekeeping.
...
// NormalizeCuration reads a stored value. Empty is open: a space that has
...
const (
	...
	// plugin in an open-mode space. It is labelled so a space's list reads as the
	// history it is rather than as one somebody curated.
	ActivationAutomatic ActivationOrigin = "automatic"
)

// Activation is one space's pinned use of one catalog plugin.
//
...
// ActivateInput pins one release for one space.
//
