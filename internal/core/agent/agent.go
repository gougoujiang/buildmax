...
func WithStreamSink(sink model.StreamSink) ProcessOption {
...
// MaxIterations sets the maximum number of loop iterations.
func MaxIterations(n int) AgentConfigurer {
	return func(a *Agent) {
		a.maxIter = n
	}
}
...
func SystemPrompt(prompt string) AgentConfigurer {
	return func(a *Agent) {
...
func (a *Agent) Process(ctx context.Context, buffer MessageBuffer, userMessage string, opts ...ProcessOption) (reply string, stats RunStats, err error) {
	...
	return a.processLoop(ctx, buffer, cfg)
}
...
func (a *Agent) ProcessAfterUserAppended(ctx context.Context, buffer MessageBuffer, opts ...ProcessOption) (reply string, stats RunStats, err error) {
