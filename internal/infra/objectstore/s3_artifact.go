...
func (s *S3ArtifactStorage) PutResult(ctx context.Context, ref RunRef, data []byte) error {
	key := ArtifactResultKey(s.prefix, ref.UserID, ref.ConversationID, ref.TaskID, ref.TaskRunID)
...
func (s *S3ArtifactStorage) PutArtifactFile(ctx context.Context, ref RunObjectRef, r io.Reader) error {
	key, err := ArtifactFileKey(s.prefix, ref.UserID, ref.ConversationID, ref.TaskID, ref.TaskRunID, ref.RelPath)
	if err != nil {
		return err
	}
