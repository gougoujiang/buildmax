...
// pluginActivationRow is one space's pinned use of one catalog plugin.
//
...
// (space_id, plugin_name) is what makes an activation one row per pair rather
...
type pluginActivationRow struct {
	...
	SpaceID    uint64 `gorm:"column:space_id;not null;uniqueIndex:ux_plugin_activation_space_plugin,priority:1"`
	PluginName string `gorm:"type:varchar(128);not null;uniqueIndex:ux_plugin_activation_space_plugin,priority:2"`
...
func (s *Store) pluginActivationSelect(ctx context.Context) *gorm.DB {
	return s.db.WithContext(ctx).Model(&pluginActivationRow{}).
		Select("plugin_activation.*, t.public_id AS space_public_id, ab.public_id AS activated_by_public_id, ub.public_id AS updated_by_public_id").
		Joins("INNER JOIN space t ON t.id = plugin_activation.space_id").
...
func (s *Store) ActivatePlugin(ctx context.Context, in coreplugin.ActivateInput) (*coreplugin.Activation, error) {
	space, err := lookupKey(ctx, s.db, "space", in.SpaceID)
	if err != nil {
		return nil, err
	}
	...
	return s.GetPluginActivation(ctx, in.SpaceID, in.PluginName)
}

// GetPluginActivation returns one space's activation of one plugin.
func (s *Store) GetPluginActivation(ctx context.Context, spaceID, pluginName string) (*coreplugin.Activation, error) {
	space, err := lookupKey(ctx, s.db, "space", spaceID)
	if err != nil {
		return nil, err
	}
	var row pluginActivationReadRow
	err = s.pluginActivationSelect(ctx).
		Where("plugin_activation.space_id = ? AND plugin_activation.plugin_name = ?", space, pluginName).
		Take(&row).Error
...
// ListPluginActivations returns a space's activations, oldest first.
func (s *Store) ListPluginActivations(ctx context.Context, spaceID string) ([]coreplugin.Activation, error) {
	space, err := lookupKey(ctx, s.db, "space", spaceID)
	if err != nil {
		return nil, err
	}
	...
	err = s.pluginActivationSelect(ctx).
		Where("plugin_activation.space_id = ?", space).
...
func (s *Store) MovePluginActivationPin(ctx context.Context, in coreplugin.MovePinInput) (*coreplugin.Activation, error) {
	return s.updatePluginActivation(ctx, in.SpaceID, in.PluginName, in.ActorID, map[string]any{
		"version": in.Version,
		"digest":  in.Digest,
	})
}
...
func (s *Store) SetPluginActivationEnabled(ctx context.Context, spaceID, pluginName string, enabled bool, actorID string) (*coreplugin.Activation, error) {
	return s.updatePluginActivation(ctx, spaceID, pluginName, actorID, map[string]any{
		"enabled": enabled,
	})
}
...
func (s *Store) updatePluginActivation(ctx context.Context, spaceID, pluginName, actorID string, fields map[string]any) (*coreplugin.Activation, error) {
	space, err := lookupKey(ctx, s.db, "space", spaceID)
	if err != nil {
		return nil, err
	}
	...
	res := s.db.WithContext(ctx).Model(&pluginActivationRow{}).
		Where("space_id = ? AND plugin_name = ?", space, pluginName).
		Updates(updates)
	if res.Error != nil {
	...
	if res.RowsAffected == 0 {
		...
		existing, getErr := s.GetPluginActivation(ctx, spaceID, pluginName)
		if getErr != nil {
			return nil, getErr
		}
		if existing == nil {
	...
	return s.GetPluginActivation(ctx, spaceID, pluginName)
}
