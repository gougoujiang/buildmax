...
func (m *mockWebFetchLLM) ChatWithTools(ctx context.Context, messages []llm.Message, tools []llm.ToolDef) (string, []llm.ToolCall, error) {
	m.mu.Lock()
	m.callCount++
	m.mu.Unlock()
...
func TestWebFetch_Execute_MissingURL(t *testing.T) {
	...
	w, err := NewWebFetch(mock, 15*time.Minute)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWebFetch_Execute_CacheExpiry(t *testing.T) {
	...
	mock := &mockWebFetchLLM{reply: "ok"}
	w, err := NewWebFetch(mock, 1*time.Millisecond)
	if err != nil {
		t.Fatal(err)
	}
	ctx := context.Background()
