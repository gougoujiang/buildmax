...
// TaskBuildmaxObjectKey returns the S3 object key for a task buildmax file (logs, sessions, settings).
// relPath is validated with CleanRelPath (no .., no absolute).
func TaskBuildmaxObjectKey(prefix, workspaceID, taskID, relPath string) (string, error) {
	clean, err := CleanRelPath(relPath)
	if err != nil {
		return "", err
	}
