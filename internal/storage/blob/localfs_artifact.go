...
type LocalFSArtifactStorage struct {
	artifactDir func(workspaceID, taskID, runID, artifactID string) string
}

// NewLocalFSArtifactStorage returns an ArtifactStorage that uses the given dir function.
func NewLocalFSArtifactStorage(artifactDir func(workspaceID, taskID, runID, artifactID string) string) *LocalFSArtifactStorage {
	return &LocalFSArtifactStorage{artifactDir: artifactDir}
}

// PutResult writes the artifact result file as result.md.
func (s *LocalFSArtifactStorage) PutResult(ctx context.Context, workspaceID, taskID, runID, artifactID string, data []byte) error {
	dir := s.artifactDir(workspaceID, taskID, runID, artifactID)
...
func (s *LocalFSArtifactStorage) GetResult(ctx context.Context, workspaceID, taskID, runID, artifactID string) ([]byte, error) {
	dir := s.artifactDir(workspaceID, taskID, runID, artifactID)
