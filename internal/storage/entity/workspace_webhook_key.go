...
// WorkspaceWebhookKey is a webhook API key for a workspace. Plaintext key is returned only at creation; only key_hash is stored.
// JSON uses snake_case per project convention.
type WorkspaceWebhookKey struct {
...
func (WorkspaceWebhookKey) TableName() string { return "workspace_webhook_key" }
