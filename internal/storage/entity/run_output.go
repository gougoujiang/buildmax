...
func (s *Store) GetTaskRunOutputFiles(ctx context.Context, chatRunID string) ([]TaskRunArtifact, error) {
	var items []TaskRunArtifact
	err := s.db.WithContext(ctx).Where("task_run_id = ?", chatRunID).Order("relative_path ASC").Find(&items).Error
	return items, err
}

// TaskRunHasOutput returns true if the run has at least one output file (and thus is a valid "artifact" for content).
func (s *Store) TaskRunHasOutput(ctx context.Context, chatRunID string) (bool, error) {
	var n int64
	err := s.db.WithContext(ctx).Model(&TaskRunArtifact{}).Where("task_run_id = ?", chatRunID).Limit(1).Count(&n).Error
	if err != nil {
		return false, err
	}
	return n > 0, nil
