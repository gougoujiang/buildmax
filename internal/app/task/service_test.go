package task

import (
	"context"
	"testing"

	"buildmax/internal/storage/entity"
	"buildmax/internal/testutil"
)

func TestCreateRun_PersistsProvenance(t *testing.T) {
	taskStore := &testutil.MockTaskStore{
		List: []entity.Task{{
			TaskID: "t_1",
			TeamID: "tm_1",
			Status: "SUCCEEDED",
		}},
	}
	runStore := &testutil.MockTaskRunStore{}
	svc := &Service{
		Tasks:    taskStore,
		TaskRuns: runStore,
	}

	run, err := svc.CreateRun(context.Background(), CreateRunCmd{
		UserID:        "u1",
		TaskID:        "t_1",
		Input:         "try again",
		CreatedByType: entity.RunCreatedByTypeUser,
		TriggerSource: entity.RunTriggerSourcePortalConversation,
	})
	if err != nil {
		t.Fatalf("CreateRun: %v", err)
	}
	if run == nil {
		t.Fatal("CreateRun returned nil run")
	}
	if len(runStore.Runs) != 1 {
		t.Fatalf("run count = %d, want 1", len(runStore.Runs))
	}
	got := runStore.Runs[0]
	if got.CreatedBy != "u1" {
		t.Fatalf("created_by = %q, want %q", got.CreatedBy, "u1")
	}
	if got.CreatedByType != entity.RunCreatedByTypeUser {
		t.Fatalf("created_by_type = %q, want %q", got.CreatedByType, entity.RunCreatedByTypeUser)
	}
	if got.TriggerSource != entity.RunTriggerSourcePortalConversation {
		t.Fatalf("trigger_source = %q, want %q", got.TriggerSource, entity.RunTriggerSourcePortalConversation)
	}
}
