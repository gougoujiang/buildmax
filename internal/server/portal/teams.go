...
func teamMemberToResponse(member entity.TeamMember, user *entity.User) teamMemberResponse {
...
func (h *Handler) addTeamMemberHandler(w http.ResponseWriter, r *http.Request) {
	...
	isOwner := false
	for i := range members {
		if members[i].UserID == userID && members[i].Role == entity.TeamRoleOwner {
			isOwner = true
			break
		}
	}
