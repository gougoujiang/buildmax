...
func llmCallsFixture(ledger *llmStubLedger) Config {
	return Config{
		...
		Conversations: &mock.MockConversationStore{
			Conversations: []coreconv.Conversation{{ID: "c_1", SpaceID: llmTestSpace, CreatedBy: llmTestUser}},
		},
		LLMCalls: ledger,
	}
}

func getLLMCalls(t *testing.T, cfg Config, spaceID, taskRunID string, auth bool) *httptest.ResponseRecorder {
	t.Helper()
	mux := http.NewServeMux()
	New(cfg).Register(mux)
	req := httptest.NewRequest(http.MethodGet, "/api/spaces/"+spaceID+"/task-runs/"+taskRunID+"/llm-calls", nil)
	if auth {
...
func TestListTaskRunLLMCalls(t *testing.T) {
	rec := getLLMCalls(t, llmCallsFixture(&llmStubLedger{calls: []coregw.Call{stagedCall()}}), llmTestSpace, "r_1", true)
...
func TestListTaskRunLLMCallsReportsCacheTokens(t *testing.T) {
	...
	rec := getLLMCalls(t, llmCallsFixture(&llmStubLedger{calls: []coregw.Call{call}}), llmTestSpace, "r_1", true)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200: %s", rec.Code, rec.Body)
	}
	var out []LLMCallSummary
...
// TestListTaskRunLLMCallsHidesOperatorRouting keeps the reader inside the space
...
func TestListTaskRunLLMCallsRefusals(t *testing.T) {
	...
	t.Run("unauthenticated", func(t *testing.T) {
		if code := getLLMCalls(t, llmCallsFixture(staged), llmTestSpace, "r_1", false).Code; code != http.StatusUnauthorized {
			t.Errorf("status = %d, want 401", code)
		}
	})

	// A member of one space must not enumerate another space's spending by naming
	// a run id, which is why the run's ownership is checked before the query.
	t.Run("a space the caller does not belong to", func(t *testing.T) {
		code := getLLMCalls(t, llmCallsFixture(staged), "tm_other", "r_1", true).Code
		if code == http.StatusOK {
			t.Error("a foreign space's run returned a ledger")
		}
	})

	t.Run("a run the server does not have", func(t *testing.T) {
		code := getLLMCalls(t, llmCallsFixture(staged), llmTestSpace, "r_missing", true).Code
	...
	t.Run("no ledger configured", func(t *testing.T) {
		...
		cfg.LLMCalls = nil
		if code := getLLMCalls(t, cfg, llmTestSpace, "r_1", true).Code; code != http.StatusServiceUnavailable {
	...
	t.Run("the query fails", func(t *testing.T) {
		...
		if code := getLLMCalls(t, cfg, llmTestSpace, "r_1", true).Code; code != http.StatusInternalServerError {
...
func TestListTaskRunLLMCallsEmptyIsAList(t *testing.T) {
	rec := getLLMCalls(t, llmCallsFixture(&llmStubLedger{}), llmTestSpace, "r_1", true)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200: %s", rec.Code, rec.Body)
	}
