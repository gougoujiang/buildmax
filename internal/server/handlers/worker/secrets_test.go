...
func secretsHandler(t *testing.T, cons agentdef.SecretConsumption, mat SecretMaterializer) (*http.ServeMux, string) {
	...
	run := coretask.Run{ID: taskRunID, TaskID: taskID, Status: "RUNNING", CreatedAt: time.Unix(1, 0).UTC()}
	agentID := "a_1"
	...
	agents := &mock.MockAgentStore{Agents: []agentdef.Agent{
		{ID: agentID, TeamID: teamID, Name: "deployer", Revision: 1, SecretConsumption: cons},
	}}
...
func TestGetTaskRunSecrets_FeatureOffIsEmpty(t *testing.T) {
	...
	runs := &mock.MockTaskRunStore{Runs: []coretask.Run{run}, TaskList: []coretask.Task{task}}
	agents := &mock.MockAgentStore{Agents: []agentdef.Agent{{ID: agentID, TeamID: teamID, Revision: 1, SecretConsumption: cons}}}
