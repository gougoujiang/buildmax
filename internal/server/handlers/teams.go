...
func (h *Handler) listTeamMembersHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i := range list {
		...
		if h.cfg.UserStore != nil {
			...
			if err != nil {
				httputil.WriteInternalError(w, err, "handler error", "handler", "list_team_members_user", "user_id", userID, "team_id", teamID, "member_user_id", list[i].UserID)
				return
			}
			user = found
		}
...
func (h *Handler) addTeamMemberHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.WriteInternalError(w, err, "handler error", "handler", "list_team_members_for_add", "user_id", userID, "team_id", teamID)
		return
	}
	isOwner := false
	for i := range members {
	...
	if err != nil {
		httputil.WriteInternalError(w, err, "handler error", "handler", "lookup_team_member_user", "team_id", teamID, "email", email)
		return
	}
	if user == nil {
...
func (h *Handler) removeTeamMemberHandler(w http.ResponseWriter, r *http.Request) {
	...
	members, err := h.cfg.TeamStore.ListTeamMembers(r.Context(), teamID)
	if err != nil {
		httputil.WriteInternalError(w, err, "handler error", "handler", "list_team_members_for_remove", "user_id", userID, "team_id", teamID)
		return
	}
	isOwner := false
	for i := range members {
