...
// space's background runs may use.
//
...
// is the authority a space's other shared automation already needs.
...
func (h *Handler) listPluginActivationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	activations, err := h.cfg.Plugins.ListActivations(r.Context(), spaceID)
	if err != nil {
		if writePluginActivationError(w, err) {
			return
		}
		httputil.WriteInternalError(w, err, "handler error", "handler", "list_plugin_activations", "user_id", userID, "space_id", spaceID)
		return
	}
...
func (h *Handler) activatePluginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := h.guard().SpaceAction(w, r, userID, spaceID, corespace.ActionManageAgents); !ok {
		return
	}
	if !h.requirePluginService(w) {
		return
	...
	if err != nil {
		...
		httputil.WriteInternalError(w, err, "handler error", "handler", "activate_plugin", "user_id", userID, "space_id", spaceID)
		return
	}
...
func (h *Handler) patchPluginActivationHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Version != nil {
		...
		if err != nil {
			...
			httputil.WriteInternalError(w, err, "handler error", "handler", "move_plugin_pin", "user_id", userID, "space_id", spaceID)
			return
		}
		current = moved
	}
	if req.Enabled != nil {
		updated, err := h.cfg.Plugins.SetActivationEnabled(r.Context(), spaceID, pluginName, *req.Enabled, userID)
		if err != nil {
			if writePluginActivationError(w, err) {
				return
			}
			httputil.WriteInternalError(w, err, "handler error", "handler", "set_plugin_activation_enabled", "user_id", userID, "space_id", spaceID)
			return
		}
		current = updated
	}
...
func (h *Handler) setPluginCurationHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.cfg.Plugins.SetCuration(r.Context(), spaceID, req.Curation, userID); err != nil {
		if writePluginActivationError(w, err) {
			return
		}
		httputil.WriteInternalError(w, err, "handler error", "handler", "set_plugin_curation", "user_id", userID, "space_id", spaceID)
		return
	}
