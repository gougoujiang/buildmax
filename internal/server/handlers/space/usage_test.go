...
func TestUsageHandler(t *testing.T) {
	...
	spaceStore := &mock.MockSpaceStore{
		Spaces: []corespace.Space{
			{ID: spaceID, Name: "My Space", PersonalForUserID: util.Ptr(userID), QuotaTier: "free_trial", CreatedBy: userID, CreatedAt: time.Now().UTC()},
		},
		Members: []corespace.Member{
			{SpaceID: spaceID, UserID: userID, Role: corespace.RoleOwner, CreatedAt: time.Now().UTC()},
		},
	}
	...
	tests := []struct {
		...
		{
			name:       "legacy usage route returns personal space usage",
			authHeader: "Bearer " + testsupport.SignJWT(userID, secret),
			path:       "/api/usage",
			checker:    checker,
			jwtSecret:  secret,
		...
		{
			name:       "space usage route returns space usage",
			authHeader: "Bearer " + testsupport.SignJWT(userID, secret),
			path:       "/api/spaces/" + spaceID + "/usage",
			checker:    checker,
			jwtSecret:  secret,
			wantStatus: http.StatusOK,
			checkBody: func(t *testing.T, body []byte) {
