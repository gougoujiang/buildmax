...
// newActivationFixture wires the real handler over in-memory stores. The space
...
func newActivationFixture(t *testing.T, curation coreplugin.Curation) *activationFixture {
	...
	spaceID := "tm_1"
	spaces := &mock.MockSpaceStore{
		Spaces: []corespace.Space{{ID: spaceID, Name: "Space", CreatedBy: "u_owner", PluginCuration: curation}},
	...
	h := New(Config{JWTSecret: activationSecret, Spaces: spaces, Plugins: svc})
	mux := http.NewServeMux()
	h.Register(mux)
	return &activationFixture{mux: mux, catalog: catalog, spaces: spaces, base: "/api/spaces/" + spaceID}
}
...
func TestActivationRoutesAuthority(t *testing.T) {
	...
	if rec.Code == http.StatusOK {
		t.Errorf("a non-member read a space's activations (status %d)", rec.Code)
	}
}
...
func TestActivationRoutesWithoutTheService(t *testing.T) {
	...
	h := New(Config{JWTSecret: activationSecret, Spaces: spaces})
	mux := http.NewServeMux()
	h.Register(mux)

	req := httptest.NewRequest(http.MethodGet, "/api/spaces/"+spaceID+"/plugin-activations", nil)
