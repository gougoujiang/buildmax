...
func auditFixture(t *testing.T) (*http.ServeMux, *mock.MockAuditStore, *mock.MockUserStore) {
	...
	h := New(Config{
		...
		Spaces: &mock.MockSpaceStore{
			Spaces: []corespace.Space{{ID: matrixSpace, Name: "Matrix", CreatedBy: matrixOwner}},
...
func TestDenialIsRecorded(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/spaces/"+matrixSpace+"/agents", strings.NewReader("{}"))
	...
	got := store.Events[0]
	if got.Action != coreaudit.AccessDenied || got.ActorID != matrixMember || got.SpaceID != matrixSpace {
...
func TestAuditTrailIsOwnerOnly(t *testing.T) {
	...
	store.Events = []coreaudit.Event{
		{ID: "ae_1", SpaceID: matrixSpace, ActorType: coreaudit.ActorUser, ActorID: matrixOwner, Action: coreaudit.SpaceMemberAdded, CreatedAt: time.Unix(1, 0).UTC()},
	}
	...
	for _, tc := range []struct {
		...
		req := httptest.NewRequest(http.MethodGet, "/api/spaces/"+matrixSpace+"/audit-events", nil)
...
func TestAuditTrailCarriesNoContent(t *testing.T) {
	mux, store, _ := auditFixture(t)
	store.Events = []coreaudit.Event{{
		ID: "ae_1", SpaceID: matrixSpace, ActorType: coreaudit.ActorUser,
