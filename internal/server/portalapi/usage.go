...
func (h *Handler) usageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.WriteInternalError(w, err, "portal handler error", "handler", "usage_get_personal_team", "user_id", userID)
		return
	}
	if team == nil {
...
func (h *Handler) writeUsageForTeam(w http.ResponseWriter, r *http.Request, teamID string) {
	...
	if err != nil {
		httputil.WriteInternalError(w, err, "portal handler error", "handler", "usage", "team_id", teamID)
		return
	}
	resp := usageResponse{
