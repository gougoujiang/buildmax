...
func TestListConversationTasksHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			taskStore:    &testutil.MockTaskStore{List: []db.Task{}},
			authHeader:   "Bearer " + testutil.SignJWT("u1", secret),
			path:         "/api/teams/" + teamID + "/conversations/" + conversationID + "/tasks",
		...
		},
		{
			...
			taskStore:    &testutil.MockTaskStore{List: []db.Task{task1, task2}},
			authHeader:   "Bearer " + testutil.SignJWT("u1", secret),
			path:         "/api/teams/" + teamID + "/conversations/" + conversationID + "/tasks",
			wantStatus:   http.StatusOK,
			wantBodyHas:  "t1",
...
func TestCreateConversationTaskHandler(t *testing.T) {
	...
	denyChecker := quota.NewChecker(
		&testutil.DenyQuotaTeamStore{Team: &db.Team{TeamID: teamID, QuotaTier: "free_trial"}},
		&testutil.DenyQuotaUsageReader{RunCount: 10, TotalTokens: 0},
		&testutil.DenyQuotaTierStore{Tier: &db.QuotaTier{TierName: "free_trial", MaxRunsPerPeriod: 10, MaxTokensPerPeriod: 100000, PeriodDays: 30}},
		"free_trial",
	)
