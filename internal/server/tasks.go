...
// Optional: limit, offset, executed_only (when true, only tasks that have been run), order (asc or desc; default desc = latest first).
...
func (s *Server) listWorkspaceTasksHandler(w http.ResponseWriter, r *http.Request) {
	...
	if usePaginated {
		...
		list, total, err := s.cfg.TaskStore.ListTasksByWorkspacePaginated(r.Context(), workspaceID, executedOnly, limit, offset)
		if err != nil {
	...
	list, err := s.cfg.TaskStore.ListTasksByWorkspace(r.Context(), workspaceID, order)
	if err != nil {
		writeInternalError(w, err, "handler", "list_tasks", "workspace_id", workspaceID)
		return
	}
...
func (s *Server) createWorkspaceTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	task, err := s.cfg.TaskStore.CreateTask(r.Context(), workspaceID, req.Input, title, userID)
	if err != nil {
