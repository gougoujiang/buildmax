...
func TestLoadAgentDefs_MaxIterations(t *testing.T) {
	dir := t.TempDir()
	content := `---
name: fast-agent
description: A fast bounded agent
tools: Read
max_iterations: 20
---

Do things quickly.`
	if err := os.WriteFile(filepath.Join(dir, "fast-agent.md"), []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
	defs, err := LoadAgentDefs(dir)
	if err != nil {
	...
	if defs[0].MaxIterations != 20 {
		t.Errorf("MaxIterations = %d, want 20", defs[0].MaxIterations)
	}
}

func TestLoadAgentDefs_MaxIterationsInvalid(t *testing.T) {
	dir := t.TempDir()
	content := `---
name: agent
...
Body.`
	...
	// Invalid value should fall back to 0 (use runner default).
	if defs[0].MaxIterations != 0 {
		t.Errorf("MaxIterations = %d, want 0 for invalid value", defs[0].MaxIterations)
	}
}
