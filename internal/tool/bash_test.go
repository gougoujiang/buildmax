...
func TestBash_Execute_truncation(t *testing.T) {
	...
	// Head (40%) + tail (40%) + note; well within maxOutputRunes+200.
	if len([]rune(out)) > maxOutputRunes+200 {
		t.Errorf("returned output rune count %d should be at most ~30k + note", len([]rune(out)))
	}
	// Both ends of the output must be present (head+tail, not just head).
	headEnd := string([]rune(out)[:100])
	tailStart := string([]rune(out)[len([]rune(out))-100:])
	if strings.Contains(headEnd, "omitted") {
		t.Error("truncation note should not be in the very first 100 runes")
	}
	if strings.Contains(tailStart, "omitted") {
		t.Error("truncation note should not be in the very last 100 runes (tail content should follow)")
	}
}
...
func TestBashCheckArgs_Catastrophic(t *testing.T) {
	...
	ask := []string{
		// rm targeting non-root paths is risky but not catastrophic → Ask
		"rm -rf /tmp/build",
		"rm -rf ./dist",
		"rm -rf ../other",
	}
	...
	for _, cmd := range deny {
		t.Run("deny: "+cmd, func(t *testing.T) {
			...
			if got := b.CheckArgs(args); got != llm.ToolActionDeny {
				t.Errorf("CheckArgs(%q) = %v; want Deny", cmd, got)
			}
		})
	}
	for _, cmd := range ask {
		t.Run("ask: "+cmd, func(t *testing.T) {
			args := map[string]any{"command": cmd}
			if got := b.CheckArgs(args); got != llm.ToolActionAsk {
				t.Errorf("CheckArgs(%q) = %v; want Ask", cmd, got)
			}
		})
	}
	for _, cmd := range allow {
		t.Run("allow: "+cmd, func(t *testing.T) {
			args := map[string]any{"command": cmd}
			if got := b.CheckArgs(args); got != llm.ToolActionAllow {
				t.Errorf("CheckArgs(%q) = %v; want Allow", cmd, got)
			}
		})
	}
}
