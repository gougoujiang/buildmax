...
// It implements the llm.Tool interface.
type WebFetch struct {
	llmClient llm.LLMClient
	cache     map[string]cacheEntry
	cacheMu   sync.RWMutex
	cacheTTL  time.Duration
	// sandbox, when set, gates outbound requests by hostname so WebFetch
	// honors the same allow-list as the bash sandbox proxy. Defaults to
	// NoopSandbox (no enforcement).
	sandbox agent.SandboxView
}

// NewWebFetch creates a WebFetch tool with the given LLM client and cache TTL.
// llmClient may be nil: fetching without a "prompt" argument still works; if "prompt"
// is set, Execute returns an error until a non-nil client is provided.
...
// WithSandbox installs v as the host filter on w and returns w. Mutates
// in place because WebFetch holds shared cache state behind a mutex —
// copying would split the cache. Pass agent.NoopSandbox{} (or nil) to
// disable enforcement.
...
func (w *WebFetch) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	// 2.5 Sandbox host check. WebFetch is a non-bash tool and is not
	// subject to the OS-level sandbox, so we enforce the same network
	// allow-list in-Go. Skips when sandbox is disabled (NoopSandbox).
	if w.sandbox != nil && w.sandbox.Enabled() {
		u, err := url.Parse(normalizedURL)
		if err == nil {
			if ok, reason := w.sandbox.HostAllowed(u.Host); !ok {
	...
	if prompt != "" {
		...
		reply, _, _, err := w.llmClient.ChatCompletionBlocking(ctx, []llm.Message{
...
func truncateContent(content string, maxRunes int) string {
	truncated := util.TruncateRunes(content, maxRunes)
	if truncated == content {
		return content
	}
	return truncated + "\n(content truncated; total " + strconv.Itoa(len([]rune(content))) + " characters)"
}
