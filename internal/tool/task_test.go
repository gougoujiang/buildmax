...
type mockRunner struct {
	reply    string
	lastOpts SubAgentRunOpts
}

func (m *mockRunner) RunSubAgent(_ context.Context, opts SubAgentRunOpts, _ string) (string, error) {
	m.lastOpts = opts
	return m.reply, nil
}

// mockAgentTool is a minimal llm.Tool for constructing AgentTypeConfig in tests.
...
func testAgentTypes() map[string]AgentTypeConfig {
	return map[string]AgentTypeConfig{
		...
		"custom-agent": {
			Tools:        []llm.Tool{&mockAgentTool{name: ToolNameGrep}},
...
func TestTaskTool_Execute_PassesMaxIterAndModel(t *testing.T) {
	runner := &mockRunner{reply: "ok"}
	...
	if runner.lastOpts.MaxIter != 10 {
		t.Errorf("MaxIter = %d, want 10", runner.lastOpts.MaxIter)
	}
	if runner.lastOpts.Model != "fast-model" {
		t.Errorf("Model = %q, want fast-model", runner.lastOpts.Model)
	}
}

func TestTaskTool_Execute_ReplyTruncation(t *testing.T) {
	// Build a reply larger than maxSubAgentReplyRunes.
	longReply := strings.Repeat("x", maxSubAgentReplyRunes+5000)
	runner := &mockRunner{reply: longReply}
	tool, err := NewTask(runner, testAgentTypes())
	if err != nil {
		t.Fatal(err)
	}
	reply, err := tool.Execute(context.Background(), map[string]any{
		...
		"prompt":        "return lots of text",
		"subagent_type": "general",
	})
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if len([]rune(reply)) >= maxSubAgentReplyRunes+5000 {
		t.Errorf("reply was not truncated: len=%d", len([]rune(reply)))
	}
	if !strings.Contains(reply, "truncated") {
		t.Error("reply should contain truncation note")
	}
}

func TestTruncateSubAgentReply_NoTruncation(t *testing.T) {
	s := strings.Repeat("a", 100)
	if got := truncateSubAgentReply(s, 200); got != s {
		t.Error("short reply should not be truncated")
	}
}

func TestTruncateSubAgentReply_HeadAndTailPresent(t *testing.T) {
	// Build a string where head and tail are distinct.
	head := strings.Repeat("H", 10000)
	middle := strings.Repeat("M", 10000)
	tail := strings.Repeat("T", 10000)
	long := head + middle + tail
	got := truncateSubAgentReply(long, maxSubAgentReplyRunes)
	if !strings.HasPrefix(got, "H") {
		t.Error("truncated reply should start with head content")
	}
	if !strings.HasSuffix(strings.TrimRight(got, "\n"), "T") {
		t.Error("truncated reply should end with tail content")
	}
