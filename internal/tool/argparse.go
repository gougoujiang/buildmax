...
// parseRequiredStringRaw extracts a required string argument from args without trimming.
// Returns an error if the key is missing, not a string, or empty.
func parseRequiredStringRaw(args map[string]any, key string) (string, error) {
	v, ok := args[key]
	if !ok {
		return "", fmt.Errorf("missing %s", key)
	}
